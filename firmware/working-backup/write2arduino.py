...
string2send = b"hello world\n"
print("Sending message: ", string2send.decode())
ard.serial.write(string2send)
line = []
try:
    if ard.serial.isOpen():
        while True:
            data = ard.serial.read().decode('cp1252').encode('utf-8').decode()
            #print(data)
            line.append(data)
            if data == "\n": 
                print("Receiving message: ", ''.join(line))
                ard.close()
                break
